$(document).ready(function(){
  ...
  var btn_kpv = [201,205,223,226,
                 233,245,250,270]
  var btn_id_on = ['crystal_on','beam_on','usb_485_on','pc_tcp_on',
                   'meter_232_on','meter_tcp_on','per_auto_on','marker_send_keystroke'];
  var btn_id_off = ['crystal_off','beam_off','usb_485_off','pc_tcp_off',
                    'meter_232_off','meter_tcp_off','per_auto_off','marker_send_comm'];
...
// websocket    
  ...
  socket.onmessage = function (message) {
    ...
    if (JSON.parse(message.data).dest == 'get_network_settings'){
      let wifi_name = JSON.parse(message.data).wifi_name
      let wifi_password = JSON.parse(message.data).wifi_password
      let ip = JSON.parse(message.data).ip
      let mask = JSON.parse(message.data).mask
      let gateway = JSON.parse(message.data).gateway
      let port = JSON.parse(message.data).port
      let ss = append_ss_wt('sec_network_settings','Network settings')
        append_row(ss)
          append_lbl(ss,1,'wifi name')
          append_input(ss,1,'wifi_name_input','text',wifi_name)
        append_row(ss)
          append_lbl(ss,2,'wifi password')
          append_input(ss,2,'wifi_password_input','text',wifi_password)            
        append_row(ss)
          append_lbl(ss,3,'eth0 ip address')
          append_input(ss,3,'ip_input','text',ip)
        append_row(ss)
          append_lbl(ss,4,'eth0 subnet mask')
          append_input(ss,4,'mask_input','text',mask)
        append_row(ss)
          append_lbl(ss,5,'eth0 gateway')
          append_input(ss,5,'gateway_input','text',gateway)
        append_row(ss)
          append_lbl(ss,6,'eth0 port')
          append_input(ss,6,'port_input','text',port)
        append_row(ss)
          append_btn(ss,7,'save_network_settings_btn','save')
          save_network_settings_btn_function()
        append_row(ss)
          append_btn(ss,7,'cancel_network_settings_btn','cancel')
          $('#cancel_network_settings_btn').click(function(){remove_element('sec_network_settings')})
    }

    if (JSON.parse(message.data).dest == 'network_settings_check'){
      let msg = JSON.parse(message.data).msg
      let success = parseInt(JSON.parse(message.data).success)
      mod_msg_box(msg)
      if (success == 1) {
        remove_element('sec_network_settings')
      }
    }
...
// -------------------------------------------------------------
  ...
  var load_s0 = function(){
    ...
    // activitiy indicator
    let sub_sec000 = append_ss('inactivity')
      append_row(sub_sec000,'row_inactivity')
        let lbl = append_lbl(sub_sec000,1,'','inactivity_indicator')
    ...
    let ss1 = append_ss('sec_user');
      ...
      append_row(ss1)
        append_btn(ss1,3,'network_settings_btn','network settings');
        network_settings_btn_function()
    // links
    ...
    sub_sec = append_ss_wt('sec_main','Main settings');
      ...
      ssr7 = append_row(sub_sec);
        append_lbl(sub_sec,7,'Crystal connection')
        btn_grp_exc_crystal = ['crystal_on','crystal_off']
        btn_grps_exc.push(btn_grp_exc_crystal)
        append_btn(sub_sec,7,'crystal_on','Enable')
        append_btn(sub_sec,7,'crystal_off','Disable')
        crystal_on_btn_function()
        crystal_off_btn_function()
      ssr8 = append_row(sub_sec);
        append_lbl(sub_sec,8,'Send markers via')
        btn_grp_exc_marker = ['marker_send_keystroke','marker_send_comm']
        btn_grps_exc.push(btn_grp_exc_marker)
        append_btn(sub_sec,8,'marker_send_keystroke','key')
        append_btn(sub_sec,8,'marker_send_comm','comm')
        marker_send_keystroke_btn_function()
        marker_send_comm_btn_function()        
    // main settings section
...
// -------------------------------------------------------------
// generate settings database section
  var load_s1 = function(){
    console.log('loading s1')
    console.log("kpv_val.length : " + kpv_val.length)
    var s = 1;
    var s_id = "s" + s;
...
// -------------------------------------------------------------
  ...
  var mod_kpv = function(id, val, type=0) {
    row = id;
    console.log("mod_kpv -> id : " + id + "  val : " + val);
    if (type == 0) {
  ...
  var meter_tcp_off_btn_function = function(){
    $("#meter_tcp_off").click(function(){
      Dict = {'dest':'tflow_tcp_connection','data':'0'}
      sendMessage({ 'dest':'qw6', 'purpose':'tflow_tcp_connection', 'data' : Dict});
      btn_on('meter_tcp_off');
      mod_kpv(245,1,1)
    });
  };

  var crystal_on_btn_function = function(){
    $("#crystal_on").click(function(){
      btn_on('crystal_on')
      mod_kpv(201,0,1)
    })
  }

  var crystal_off_btn_function = function(){
    $("#crystal_off").click(function(){
      btn_on('crystal_off')
      mod_kpv(201,1,1)
    })
  }  

  var marker_send_keystroke_btn_function = function(){
    $("#marker_send_keystroke").click(function(){
      btn_on('marker_send_keystroke')
      mod_kpv(270,0,1)
    })
  }

  var marker_send_comm_btn_function = function(){
    $("#marker_send_comm").click(function(){
      btn_on('marker_send_comm')
      mod_kpv(270,1,1)
    })
  }  
  ...
  var controls_btn_function = function() {
    $('#controls_btn').click(function(){
      ...
      var lt = sw - ww
      window.open(base_addr + '/controls','_blank','width=' + ww + ', height=' + wh + ', left=' + lt)
    });
  };
...
///////////////////////////////////////////////////////////////
  ...
  var append_lbl = function(ss,row,text,id=''){
  ...
  function network_settings_btn_function() {
    $('#network_settings_btn').click(function(){
      if (!toggle_element('sec_network_settings')) {
        ajax_request({'purpose':'get_network_settings'})
      }
    })
  }

  function save_network_settings_btn_function() {
    $('#save_network_settings_btn').click(function() {
      let wn = document.getElementById('wifi_name_input').value
      let wp = document.getElementById('wifi_password_input').value
      let inhibit = check_input('wifi_name_input')
      inhibit += check_input('wifi_password_input')
      if (inhibit == 0) {
        ajax_request({'purpose':'network_settings_check','wifi_name':wn,'wifi_password':wp})
      }
    })
  }
  ...
  var logout_btn_function = function() {
    $('#logout_btn').click(function(){
      let logout_user = get_active_user();
      $.ajax({url:'/settings',type:'POST',data:{'purpose':'logout','user':logout_user}})
      mod_msg_box('Logging out user ' + logout_user)
      setTimeout(function(){window.open('/login','_self')},2000)
    })
  }
  ...
  var check_input = function(ele,type='string'){
    inf = document.getElementById(ele).value
    let inhibit = 0
    if (inf == null || inf == "") {
        mod_msg_box("Fill in all necessary fields");
        inhibit = 1
    }
    if (inf.length < 5 && inhibit == 0 && type !='port'){
      mod_msg_box('must be longer than 5 characters')
      inhibit = 1
    }
    if (inhibit == 0) {
      if(type == 'int') {
        if (/'\D+'/.test(inf)) {
          mod_msg_box('must be a positive integer')
          inhibit = 1
        }
      }
      else if(type == 'port') {
        if (/\D+/.test(inf) || inf.length > 5 || inf.length < 4) {
          mod_msg_box('Port must be a positive number between 1111 and 99999')
          inhibit = 1
        }
      }  
        ...
        decf = inf.indexOf('.')
        if (decf > 0) {
          oct1 = nstr.slice(0,decf)
          cts += valid_octet(oct1)
          nstr = nstr.slice(decf + 1)
          decf = nstr.indexOf('.')
          if (decf > 0) {
            oct2 = nstr.slice(0,decf)
            cts += valid_octet(oct2)
            nstr = nstr.slice(decf + 1)
            decf = nstr.indexOf('.')
            if (decf > 0) {
              oct3 = nstr.slice(0,decf)
              cts += valid_octet(oct3)
              oct4 = nstr.slice(decf + 1)
              cts += valid_octet(oct4)
            }
          }
        }
        if (cts == 4){
          let test1 = /\D+/ 
          if (!test1.test(oct1) && !test1.test(oct2) && !test1.test(oct3) && !test1.test(oct4)) {
            inhibit = 0
          }
          else {
            mod_msg_box('bad ip address format')
            inhibit = 1
          }
        }
        else {
    ...
    if (inhibit == 1){
      $('#' + ele) .prop('style','border-width:1px;border-color:red;')
    }
    else {
      $('#' + ele) .prop('style','border-width:0px;border-color:black;')     
    }
    return inhibit
  }
